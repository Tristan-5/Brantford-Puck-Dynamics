...
def _single_end_net(x_goal_line: float,
    ...
    # Corner wrap panels: netting runs around the rounded corners and stops
    # where side-board straightaways begin (x = +/- (half_len - corner_r)).
    x_side_start = sign * (rink_half_len - corner_r)
    x_wrap_min = min(x_inner_plane, x_side_start)
    x_wrap_max = max(x_inner_plane, x_side_start)

    # North corner wrap
    geom.append(Box3D.from_bounds(
        x_min=x_wrap_min,
        x_max=x_wrap_max,
        y_min=rink_half_wid - THICKNESS,
        y_max=rink_half_wid,
        z_min=z0,
        z_max=z1,
    ))
