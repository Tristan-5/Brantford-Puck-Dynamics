...
def _single_trapezoid(x_goal_line: float, half_wid: float) -> List[Box3D]:
    ...
    geom.append(
        Box3D.from_bounds(
            ...
            y_min=-y_half_front - LINE_W / 2,
            y_max=-y_half_back  + LINE_W / 2,
            z_min=z0,
            z_max=z1,
        )
    )
    ...
    geom.append(
        Box3D.from_bounds(
            x_min=min(x_front, x_back),
            x_max=max(x_front, x_back),
            y_min=+y_half_back  - LINE_W / 2,
            y_max=+y_half_front + LINE_W / 2,
            z_min=z0,
            z_max=z1,
        )
    )
    ...
    geom.append(
        Box3D.from_bounds(
            x_min=x_back - LINE_W / 2 * sign,
            x_max=x_back + LINE_W / 2 * sign,
            y_min=-y_half_back,
            y_max=+y_half_back,
            z_min=z0,
            z_max=z1,
